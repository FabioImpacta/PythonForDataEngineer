...
def data_clean(df, metadados):
    ...
    df = utils.convert_data_type(df, metadados["tipos_formatted"])
...
if __name__ == "__main__":    
    ...
    logger.info(f'Validação de Chaves; {datetime.datetime.now()}')
    # utils.keys_check(df, metadados["cols_chaves"])
    # logger.info(f'Criação de Novos Campos; {datetime.datetime.now()}')
    # df = feat_eng(df)
    # logger.info(f'Salvar dados no banco; {datetime.datetime.now()}')
    # save_data_sqlite(df)
